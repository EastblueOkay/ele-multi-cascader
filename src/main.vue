      ...
      <div class="cascader-menu-wrapper" v-click-outside="hidePopover">
        <ul class="el-cascader-menu cascader-menu" v-for="(cas, index) in casTree" :key="index">
          <li
            ...
            @click="spreadNext(item, index, $event)"
            v-for="(item, itemIdx) in cas"
            :key="itemIdx"
          >
            <el-checkbox
              :class="{'notLeaf': item.children}"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 展开下一级
      spreadNext(item, index, e) {
        const children = item.children;
        // 如果是叶子元素 则选中
        if (!children && e.target.className.indexOf('el-checkbox') === -1) {
          // 如果点击的是li
          this._liClick = true;
...
<style>
  .cascader-menu-wrapper .el-cascader-menu__item {
    padding: 8px 10px;
  }

  .cascader-menu-wrapper label.notLeaf {
    display: none;
  }
